...
class MainViewModel(
    ...
    init { getBreakingNews("us") }

    private fun getBreakingNews(countryCode: String) = viewModelScope.launch { safeBreakingNewsCall(countryCode) }

    fun searchNews(searchQuery: String) = viewModelScope.launch { safeSearchNewsCall(searchQuery) }
        ...
        if(response.isSuccessful) {
            response.body()?.let { resultResponse ->
                if(breakingNewsResponse == null) {
                    ...
                    resultResponse.articles.forEach { article -> saveArticle(article) }
    ...
    private fun saveArticle(article: Article) = viewModelScope.launch { newsRepository.upsert(article) }
